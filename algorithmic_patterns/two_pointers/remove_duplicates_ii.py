...
def remove_duplicates(nums: []) -> int:
    ...
    for i in range(len(nums)):
        count = count + 1 if i + 1 < len(nums) and nums[i] == nums[i+1] else 1
        nums[i], nums[cur_idx] = nums[cur_idx], nums[i]
        cur_idx = cur_idx + 1 if count <= 2 else cur_idx
    return cur_idx


def remove_duplicates_alt(nums: []) -> int:
...
print(remove_duplicates_alt([1,1,1,2,2,3]))
# print(remove_duplicates([0,0,1,1,1,1,2,3,3]))
# print(remove_dup([0,0,1,1,1,1,2,3,3]))